...
IF ISNULL(OBJECTPROPERTY(OBJECT_ID(N'Insurance_WorkOrderMain_Get_By_Keyword'), N'IsProcedure'), 0) = 1
	DROP PROCEDURE [dbo].[Insurance_WorkOrderMain_Get_By_Keyword];
GO
...
-- ============================================================================
CREATE PROCEDURE [dbo].[Insurance_WorkOrderMain_Get_By_Keyword] ( @Filter VARCHAR(50), @StartIndex INT = 0, @EndIndex INT = 501 )
...
BEGIN
	...
	SET @SeekFilter = '%' + ISNULL(@Filter, '') + '%'; -- contains the specified work order search keyword
	...
	(
		...
		INNER JOIN [dbo].[Insurance_WorkOrder] WO WITH (READCOMMITTED) ON ( WO.[ORDER_ID] = WOM.[Order_id] ) AND ( WO.[search_code_value] LIKE @SeekFilter )
	)
